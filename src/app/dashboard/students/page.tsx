...
import { type Student } from "@/hooks/use-students"; // Corrected import path for Student type
...
const StudentsPageContent: React.FC = () => {
  ...
  // Added 'program' to formData initial state
  const [formData, setFormData] = useState<Omit<Student, 'id'> & { program: string }>({ name: '', studentId: '', program: '' });
  ...
  const handleAddClick = () => {
    ...
    setFormData({ name: '', studentId: '', program: '' }); // Reset form data including program
    setIsDialogOpen(true);
  };

  const handleEditClick = (student: Student) => {
    setEditingStudent(student);
    setFormData(student); // Set form data from existing student including program
    setIsDialogOpen(true);
  };
    ...
    try {
      ...
      toast({
       ...
       toast({
        ...
        description: `No se pudo eliminar al estudiante. ${(error as Error).message}`,
      });
    } finally {
  ...
  const handleSaveChanges = async () => {
    if (!formData.name || !formData.studentId || !formData.program) { // Validate program field
      toast({
        variant: "destructive",
    ...
    try {
      ...
      toast({
        ...
        description: `No se pudieron guardar los cambios. ${(error as Error).message}`,
      });
    } finally {
      setIsSubmitting(false);
    }
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="sm:max-w-[425px]">
              ...
              <div className="grid gap-4 py-4">
                ...
                {/* New input field for Program */}
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="program" className="text-right">Programa</Label>
                  <Input id="program" value={formData.program} onChange={handleFormChange} className="col-span-3" placeholder="Ej: Ingeniería de Software" disabled={isSubmitting} />
                </div>
              </div>
              <DialogFooter>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Programa</TableHead> {/* New table header for Program */}
              ...
              {isLoading ? (
                Array.from({ length: 5 }).map((_, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell><Skeleton className="h-5 w-24" /></TableCell> {/* Skeleton for Program */}
                ...
                students.map((student) => {
                  ...
                  return (
                    <TableRow key={student.id}>
                      ...
                      <TableCell>{student.program}</TableCell> {/* Display program */}
                      <TableCell>
                        {average !== 'N/A' ? (
                 ...
                 <TableRow>
                    <TableCell colSpan={isAdmin ? 5 : 4} className="text-center text-muted-foreground"> {/* Adjusted colspan */}
